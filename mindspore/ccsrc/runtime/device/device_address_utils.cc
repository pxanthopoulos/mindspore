...
void DeviceAddressUtils::MallocForInput(const DeviceContext *device_context, const tensor::TensorPtr &tensor) {
  ...
  device::tracker::CALL_MEMORY_TRACKER_WITH_FILE(AddMemInfo, "PyNative",
                                                 device::tracker::MemType::kPyNativeInput, device_address->GetSize(),
                                                 device_address->kernel_tensor().get());
...
void DeviceAddressUtils::MallocForOutputs(const DeviceContext *device_context,
  ...
  for (const auto &output : outputs) {
    ...
    device::tracker::CALL_MEMORY_TRACKER_WITH_FILE(AddTask, "PyNative", "", "");
    device::tracker::CALL_MEMORY_TRACKER_WITH_FILE(AddMemInfo, "PyNative",
                                                   device::tracker::MemType::kPyNativeOutput, device_address->GetSize(),
                                                   device_address->kernel_tensor().get());
    if (!device_context->device_res_manager_->AllocateMemory(device_address.get())) {
      MS_LOG(EXCEPTION) << "Allocate memory failed";
    }
